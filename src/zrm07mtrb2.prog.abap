...
REPORT zrm07mtrb2 MESSAGE-ID m7.
...
DATA: BEGIN OF xtab6 OCCURS 0,
         ...
         ststk LIKE zzst_sto_details-ststk,
         chng(01), "FLAG
      END OF xtab6.
...
*
 LOOP AT bestand.
    ls_seltab-selname = 'S_MATNR'.  " Material
    ls_seltab-kind    = 'S'.
    ls_seltab-sign    = 'I'.
    ls_seltab-option  = 'EQ'.
    ls_seltab-low     = bestand-matnr.
    APPEND ls_seltab TO lt_seltab.

    ls_seltab-selname = 'S_WERKS'.  " Receiving Plant
    ls_seltab-kind    = 'S'.
    ls_seltab-sign    = 'I'.
    ls_seltab-option  = 'EQ'.
    ls_seltab-low     = bestand-werks.
    APPEND ls_seltab TO lt_seltab.

    ls_seltab-selname = 'S_RESWK'.  " Issuing Plant
    ls_seltab-kind    = 'S'.
    ls_seltab-sign    = 'I'.
    ls_seltab-option  = 'EQ'.
    ls_seltab-low     = bestand-reswk.
    APPEND ls_seltab TO lt_seltab.

    ls_seltab-selname = 'S_EBELN'.  " Issuing Plant
    ls_seltab-kind    = 'S'.
    ls_seltab-sign    = 'I'.
    ls_seltab-option  = 'EQ'.
    ls_seltab-low     = bestand-ebeln.
    APPEND ls_seltab TO lt_seltab.
  ENDLOOP.

  SUBMIT Z6SD020R_STO_DETAILS_BATCH_3 WITH SELECTION-TABLE lt_seltab." AND RETURN.
