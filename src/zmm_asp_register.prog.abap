...
* Purpose    - Simplification list - Functionality not available:'CURRENCY_AMOUNT_SAP_TO_DISPLAY'
* Solution   - 'CURRENCY_AMOUNT_SAP_TO_DISPLAY'
* TR         - SBXK900030 - S4H:BM:Simplification List:03.10.2018
*&---------------------------------------------------------------------*

REPORT zmm_asp_register NO STANDARD PAGE HEADING
                           LINE-SIZE  210
                           MESSAGE-ID zmm01.

*----------------------------------------------------------------------*
* OBJECT DESCRIPTION: Purchase Order Payments
* Copied from Z6MM024R_PO_PAYMENTS_A report
*----------------------------------------------------------------------*
*----------------------------------------------------------------------*
*       1 :   Tables Defination                                        *
*----------------------------------------------------------------------*
...
TYPES:BEGIN OF ty_bset,
        ...
        hwbas TYPE bset-hwbas,  " base amount
        hwste TYPE bset-hwste,  " tax amount
        fwste TYPE bset-fwste,  " tax amount
        ktosl TYPE bset-ktosl,
        knumh TYPE bset-knumh,
        stceg TYPE bset-stceg,
        kschl TYPE bset-kschl,
        kbetr TYPE bset-kbetr,  " tax rate = ( i.e., KBETR / 10 )
...
* For inv due date                   "Added by Anees 2/04/2011
        ...
        xblnr        TYPE xblnr1,
**  Added by NK on 25.10.2017
        bupla        TYPE bseg-bupla,
        bupla_txt    TYPE j_1bbranch-name,
        mwskz        TYPE bset-mwskz,  " tax code
        kbetr        TYPE bset-kbetr,  " tax rate
        hwbas        TYPE bset-hwbas,  " base amt
        hwste_igst   TYPE bset-hwste,  " tax amt
        hwste_cgst   TYPE bset-hwste,  " tax amt
        hwste_sgst   TYPE bset-hwste,  " tax amt
        stcd3        TYPE lfa1-stcd3,  " GSTN
        xblnr1       TYPE bkpf-xblnr,  " Invoice no. (reference)
      END   OF iekbe.
...
DATA: BEGIN OF iheader  OCCURS 1,
      ...
      END   OF iheader.
DATA: BEGIN OF iekbe_fin OCCURS 0,
        ebeln        LIKE  ekbe-ebeln,
        ebelp        LIKE  ekbe-ebelp,
        lifnr        LIKE ekko-lifnr,           "Vendor code
        name1        LIKE lfa1-name1,           "Vendor Name
...
**  Added by NK on 25.10.2017
        ...
        kbetr        TYPE bset-kbetr,  " tax rate
        hwbas        TYPE bset-hwbas,  " base amt
        hwste_igst   LIKE bset-hwste,  " tax amt
        hwste_cgst   LIKE bset-hwste,  " tax amt
        hwste_sgst   LIKE bset-hwste,  " tax amt
        stcd3        TYPE lfa1-stcd3,  " GSTN
        xblnr1       TYPE bkpf-xblnr,  " Invoice no. (reference)
        buzei3       TYPE bset-buzei,
      END   OF iekbe_fin.
DATA: wa_iekbe_fin LIKE LINE OF iekbe_fin.
DATA: tot_hwste_cgst LIKE bset-hwste,
      lv_hwste_cgst  LIKE bset-hwste,
      tot_hwste_sgst LIKE bset-hwste,
      lv_hwste_sgst  LIKE bset-hwste,
      tot_hwste_igst LIKE bset-hwste,
      lv_hwste_igst  LIKE bset-hwste,
      lv_belnr2      LIKE ekbe-belnr.
DATA: BEGIN OF iekbe_fin2 OCCURS 0,
        ebeln        LIKE  ekbe-ebeln,
        ebelp        LIKE  ekbe-ebelp,
        lifnr        LIKE ekko-lifnr,           "Vendor code
        name1        LIKE lfa1-name1,           "Vendor Name
...
**  Added by NK on 25.10.2017
        ...
        buzei3       TYPE bset-buzei,
      END   OF iekbe_fin2.
DATA: wa_iekbe_fin2 LIKE LINE OF iekbe_fin2.
DATA: BEGIN OF iekbe_fin3 OCCURS 0,
        ebeln        LIKE  ekbe-ebeln,
        ebelp        LIKE  ekbe-ebelp,
        lifnr        LIKE ekko-lifnr,           "Vendor code
        name1        LIKE lfa1-name1,           "Vendor Name
...
**  Added by NK on 25.10.2017
        ...
        buzei3       TYPE bset-buzei,
      END   OF iekbe_fin3.
DATA: wa_iekbe_fin3 LIKE LINE OF iekbe_fin3.
DATA: gv_iekbe_bset LIKE sy-tabix,gv_index1 LIKE sy-index, gv_tabix1 LIKE sy-tabix.
*----------------------------------------------------------------------*
*       3 :   Data Declaration                                         *
*----------------------------------------------------------------------*
TYPE-POOLS : slis.
DATA: fieldtab    TYPE slis_t_fieldcat_alv,
      ls_fieldtab TYPE slis_fieldcat_alv,
...
DATA: gv_bupla_txt TYPE j_1bbranch-name.
*----------------------------------------------------------------------*
*       4 :   Selection Screen                                         *
*----------------------------------------------------------------------*
...
PARAMETERS: r_head RADIOBUTTON GROUP g1 DEFAULT 'X',
            r_item RADIOBUTTON GROUP g1.
...
*    'IEKBE' 'INVDT' 'Inv.Date'   'BUDAT'   'EKBE' 18 fieldtab,    "document date and Invoice date is interchanged
    ...
    'IEKBE' 'IRBVL' 'Invoice Value'   'WRBTR'   'EKBE' 20 fieldtab,
...
*    'IEKBE' 'PAY_DUE_DATE' 'Pay.Due.Date' 'BUDAT' 'BSIK' 32 fieldtab, "" added by sachin""""commented not applicable
    ...
    'IEKBE' 'BUPLA' 'Place of Supply' 'BUPLA' 'BSEG' 35 fieldtab ,
    'IEKBE' 'BUPLA_TXT' 'Place of Supply Name(Receiving Location)' 'NAME' 'J_1BBRANCH' 36 fieldtab,
    'IEKBE' 'MWSKZ' 'Tax Code' 'MWSKZ' 'BSET' 37 fieldtab,
    'IEKBE' 'KBETR' 'Tax Rate' 'KBETR' 'BSET' 38 fieldtab,
    'IEKBE' 'HWBAS' 'Base Amount' 'HWBAS' 'BSET' 39 fieldtab,
    'IEKBE' 'HWSTE_IGST' 'Integrated Tax Amount' 'HWSTE' 'BSET' 40 fieldtab,
    'IEKBE' 'HWSTE_CGST' 'Central Tax Amount' 'HWSTE' 'BSET' 41 fieldtab,
    'IEKBE' 'HWSTE_SGST' 'State Tax Amount' 'HWSTE' 'BSET' 42 fieldtab,
    'IEKBE' 'STCD3' 'GSTN of Supplier' 'STCD3' 'LFA1' 43 fieldtab,
    'IEKBE' 'XBLNR1' 'Invoice Number' 'XBLNR' 'BKPF' 44 fieldtab,
    'IEKBE' 'BUZEI3' 'Tax Item no.' 'BUZEI' 'BSET' 45 fieldtab.
  text = 'GSTR 2 Report Details'.

  PERFORM build_layout.
  PERFORM build_events CHANGING it_events[].
  PERFORM build_comment CHANGING it_listheader[].
*  break 10106.
  SORT iekbe BY belnr2 gjahr2.
  DELETE ADJACENT DUPLICATES FROM iekbe[] COMPARING belnr2 gjahr2.

  IF iekbe[] IS NOT INITIAL.
    DELETE iekbe WHERE belnr2 IS INITIAL.
    SELECT bukrs belnr gjahr buzei mwskz hkont shkzg hwbas hwste fwste ktosl knumh stceg kschl kbetr bupla
        FROM bset
        INTO TABLE it_bset
        FOR ALL ENTRIES IN iekbe
        WHERE belnr EQ iekbe-belnr2
          AND bukrs IN s_bukrs                              "EQ '1000'
          AND gjahr EQ iekbe-gjahr2.

    SORT iekbe BY belnr2.
    SORT it_bset BY belnr.

    LOOP AT iekbe.
      READ TABLE it_bset INTO wa_bset WITH KEY belnr = iekbe-belnr2 BINARY SEARCH.
      IF sy-subrc EQ 0.
        MOVE-CORRESPONDING iekbe TO iekbe_fin.
        gv_iekbe_bset = sy-tabix.
        LOOP AT it_bset INTO wa_bset FROM gv_iekbe_bset.
          IF wa_bset-belnr NE iekbe-belnr2.
            EXIT.
          ELSE.
            IF iekbe-belnr2 IS NOT INITIAL.
              IF wa_bset-kschl EQ 'JIIG' OR wa_bset-kschl EQ 'JIMD'.
                READ TABLE it_bset INTO wa_bset WITH KEY bukrs = it_bseg-bukrs belnr = iekbe-belnr2 gjahr = iekbe-gjahr2 buzei = wa_bset-buzei.
*               kschl = 'JIIG'. " JIMD
*              IF sy-subrc EQ 0.
                iekbe_fin-mwskz = wa_bset-mwskz.
                iekbe_fin-kbetr = wa_bset-kbetr / 10.
                iekbe_fin-hwste_igst = wa_bset-hwste.
                iekbe_fin-hwbas = wa_bset-hwbas.
                iekbe_fin-buzei3 = wa_bset-buzei.

                IF wa_bset-bupla IS NOT INITIAL.
                  iekbe_fin-bupla = wa_bset-bupla.
                  CLEAR gv_bupla_txt.
                  SELECT SINGLE name FROM j_1bbranch INTO gv_bupla_txt WHERE bukrs = wa_bset-bukrs AND branch = wa_bset-bupla .
                  iekbe_fin-bupla_txt = gv_bupla_txt.
                ENDIF.
              ELSE.
                READ TABLE it_bset INTO wa_bset WITH KEY bukrs = it_bseg-bukrs belnr = iekbe-belnr2 gjahr = iekbe-gjahr2 buzei = wa_bset-buzei
                 kschl = 'JICG'.
*                IF sy-subrc EQ 0.
                iekbe_fin-mwskz = wa_bset-mwskz.
                iekbe_fin-kbetr = wa_bset-kbetr / 10.
                iekbe_fin-hwste_cgst = wa_bset-hwste.
                iekbe_fin-hwbas = wa_bset-hwbas.
                iekbe_fin-buzei3 = wa_bset-buzei.
                ...
                READ TABLE it_bset INTO wa_bset WITH KEY bukrs = it_bseg-bukrs belnr = iekbe-belnr2 gjahr = iekbe-gjahr2 buzei = wa_bset-buzei
                 kschl = 'JISG'.
*                  IF sy-subrc EQ 0.
                iekbe_fin-hwste_sgst = wa_bset-hwste.
                IF wa_bset-bupla IS NOT INITIAL.
                  iekbe_fin-bupla = wa_bset-bupla.
                  CLEAR gv_bupla_txt.
                  SELECT SINGLE name FROM j_1bbranch INTO gv_bupla_txt WHERE bukrs = wa_bset-bukrs AND branch = wa_bset-bupla .
...
*                ENDIF.
              ENDIF.
            ENDIF.
            IF iekbe_fin-lifnr IS NOT INITIAL.
              CLEAR iekbe_fin-stcd3.
              SELECT SINGLE stcd3 FROM lfa1 INTO iekbe_fin-stcd3 WHERE lifnr = iekbe_fin-lifnr.
            ENDIF.
*            APPEND iekbe.
            IF iekbe-belnr2 IS NOT INITIAL.
              CLEAR: iekbe_fin-xblnr1.
              SELECT SINGLE xblnr FROM bkpf INTO iekbe_fin-xblnr1 WHERE bukrs IN s_bukrs
                                          AND belnr = iekbe-belnr2
                                          AND gjahr = iekbe-gjahr2.
            ENDIF.
*            iekbe_fin-xblnr1 = iekbe-xblnr.
            IF iekbe_fin-buzei3 = '001'.
*              base amount - hwbas
            ELSE.
              CLEAR: iekbe_fin-irbvl, iekbe_fin-irlvl,
                     iekbe_fin-hwbas, iekbe_fin-kbetr,
                     iekbe_fin-mwskz, iekbe_fin-lifnr.
            ENDIF.
            APPEND iekbe_fin.
          ENDIF.
        ENDLOOP.
  ...
  ENDIF.
  IF r_head = 'X'.
    LOOP AT iekbe_fin INTO wa_iekbe_fin.
      IF wa_iekbe_fin-buzei3 = '001'.
        IF wa_iekbe_fin-belnr2 NE lv_belnr2.
          CLEAR:lv_hwste_sgst, lv_hwste_cgst, lv_hwste_igst,
          tot_hwste_sgst, tot_hwste_cgst, tot_hwste_igst,
          iekbe_fin-hwste_cgst, iekbe_fin-hwste_sgst,
          iekbe_fin-hwste_igst.
        ENDIF.
        lv_belnr2 = wa_iekbe_fin-belnr2.
        IF wa_iekbe_fin-belnr2 EQ lv_belnr2.
          lv_hwste_sgst = wa_iekbe_fin-hwste_sgst.
          tot_hwste_sgst = tot_hwste_sgst + lv_hwste_sgst.
          wa_iekbe_fin-hwste_sgst = tot_hwste_sgst.

          lv_hwste_cgst = wa_iekbe_fin-hwste_cgst.
          tot_hwste_cgst = tot_hwste_cgst + lv_hwste_cgst.
          wa_iekbe_fin-hwste_cgst = tot_hwste_cgst.

          lv_hwste_igst = wa_iekbe_fin-hwste_igst.
          tot_hwste_igst = tot_hwste_igst + lv_hwste_igst.
          wa_iekbe_fin-hwste_igst = tot_hwste_igst.
        ENDIF.
      ELSE.
        IF wa_iekbe_fin-belnr2 NE lv_belnr2.
          CLEAR:lv_hwste_sgst, lv_hwste_cgst, lv_hwste_igst,
        ...
        CLEAR: iekbe_fin-irbvl, iekbe_fin-irlvl,
               ...
               iekbe_fin-mwskz, iekbe_fin-lifnr.
      ENDIF.
      MODIFY iekbe_fin FROM wa_iekbe_fin TRANSPORTING hwste_igst hwste_cgst hwste_sgst.
    ENDLOOP.

    CLEAR: lv_hwste_igst, wa_iekbe_fin.
    LOOP AT iekbe_fin INTO wa_iekbe_fin.
      IF wa_iekbe_fin-buzei3 NE '001'.
        lv_hwste_igst = wa_iekbe_fin-hwste_igst.
        lv_belnr2 = wa_iekbe_fin-belnr2.
        READ TABLE iekbe_fin INTO wa_iekbe_fin WITH KEY belnr2 = wa_iekbe_fin-belnr2 buzei3 = '001'.
        IF sy-subrc EQ 0.
          gv_index1 = sy-index.
          gv_tabix1 = sy-tabix.
          wa_iekbe_fin-hwste_igst = lv_hwste_igst.
          MODIFY iekbe_fin FROM wa_iekbe_fin INDEX gv_tabix1 TRANSPORTING lifnr name1 matnr maktx belnr2 gjahr2
          inddt irbvl irlvl bupla bupla_txt mwskz kbetr hwbas stcd3 xblnr1 buzei3 hwste_igst.
        ENDIF.
      ENDIF.
    ENDLOOP.

    SORT iekbe_fin BY belnr2 buzei3.
    LOOP AT iekbe_fin.
      MOVE-CORRESPONDING iekbe_fin TO iekbe_fin2.
      APPEND iekbe_fin2.
      CLEAR iekbe_fin2.
    ENDLOOP.
    DELETE iekbe_fin2 WHERE lifnr IS INITIAL.

**  This report output has to be on Posting date
**  but in some cases Document Date is being fetch in query & in others Posting date is being queried & populated into field INDDT(Inv.Date)
**  So that is the reason why I'm filtering date in the end here !!
    LOOP AT iekbe_fin2 WHERE inddt IN s_bedat.  " Invoice Date
      MOVE-CORRESPONDING iekbe_fin2 TO iekbe_fin3.
      APPEND iekbe_fin3.
      CLEAR iekbe_fin3.
    ENDLOOP.
  ENDIF.
