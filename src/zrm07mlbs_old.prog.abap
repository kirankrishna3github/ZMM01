...
*   CHANGE ON: 6/10/2015
*   REASON FOR CHANGE: Authorization Check for Plant & Sales District
*   REQUEST #: IRDK920892
* --------------------------------------------------------------------------------------------*
* REVISION HISTORY----------------------------------------------------------------------------*
*
...
*
*   CHANGED BY: Chirag Shah
*   CHANGE ON: 19/10/2015
*   REASON FOR CHANGE: ZCITYC Authorization
*   REQUEST #: IRDK921161
* --------------------------------------------------------------------------------------------*
* REVISION HISTORY----------------------------------------------------------------------------*
*
...
*   REASON FOR CHANGE: Remove ZCITYC Authorization
*   REQUEST #: IRDK922028
* --------------------------------------------------------------------------------------------*
************************************************************************
* Data declarations
...
* CHANGES BY PUNAM TO ADD FOLLOWING THREE FIELDS
        ...
        zdate(08) TYPE c,      """expiry date                         "sachin
...
* END BY PUNAM
        ...
        konda     TYPE t188-konda, " Price group (customer)
        kbetr     TYPE konp-kbetr,   " Rate (condition amount or percentage) where no scale exists
        konda_txt TYPE t188t-vtext,   " PRICE GRP TXT
        atwrt_pg  TYPE ausp-atwrt, " Price group (customer) updated in msc2n for existing batches
        atwrt_txt TYPE t188t-vtext,   " PRICE GRP TXT  updated in msc2n for existing batches
        mvgr5     TYPE mvke-mvgr5, "  material group 5 , indicator for bulk branded product
...
DATA: BEGIN OF header OCCURS 0,
        ...
        konda     TYPE t188-konda, " Price group (customer)
        kbetr     TYPE konp-kbetr,   " Rate (condition amount or percentage) where no scale exists
        konda_txt TYPE t188t-vtext,   "
        atwrt_pg  TYPE ausp-atwrt, " Price group (customer) updated in msc2n for existing batches
        atwrt_txt TYPE t188t-vtext,   " PRICE GRP TXT  updated in msc2n for existing batches
        mvgr5     TYPE mvke-mvgr5, "  material group 5 , indicator for bulk branded product
        mvgr5_txt TYPE tvm5t-bezei,
        lgpbe     TYPE mard-lgpbe,   "storage bin added by pravin date - 25/09/15
        zfkimg    TYPE z6sd038t_depo-zfkimg.
DATA: END OF header.
...
* CHANGES BY PUNAM TO ADD FOLLOWING THREE FIELDS
        hsdat       TYPE mch1-hsdat, " Date of Manufacture                  "PUNAM
        vfdat       TYPE mch1-vfdat, "Shelf Life Exp. Date                  "PUNAM
        ktx         TYPE t247-ktx,   """month name                          ""sachin
        zdate1(08)  TYPE c,     ""month name year                      ""sachin
        ...
        konda       TYPE t188-konda, " Price group (customer)
        kbetr       TYPE konp-kbetr,   " Rate (condition amount or percentage) where no scale exists
        konda_txt   TYPE t188t-vtext,   " Condition type
        atwrt_pg    TYPE ausp-atwrt, " Price group (customer) updated in msc2n for existing batches
        atwrt_txt   TYPE t188t-vtext,   " PRICE GRP TXT  updated in msc2n for existing batches
        other_kbetr TYPE konp-kbetr,   " Rate (condition amount or percentage) for MSC2n/COR2 PG
        mvgr5       TYPE mvke-mvgr5, "  material group 5 , indicator for bulk branded product
        mvgr5_txt   TYPE tvm5t-bezei,
        lgpbe       TYPE mard-lgpbe, "storage bin added pravin 25/9/15
        zfkimg      TYPE z6sd038t_depo-zfkimg.
* END BY PUNAM
DATA:  END OF bestand.
...
SELECT-OPTIONS:
  ...
  werks  FOR t001l-werks  MEMORY ID wrk,
  cityc  FOR t005g-cityc,                                   "718285
...
*    exporting
*      titel              = 'Obsolete Program!'
*      textline1          = 'This program/transaction is obsolete.'
*      textline2          = 'Redirecting to standard transaction MB52.'.
*
*initialization.
*  leave to transaction 'MB52'.
...
*  ENDIF.                                                    "n577268
*
*  PERFORM check_auth_obj. " Added by Chirag Shah 19/10/2015 Check Authority of Plant & Sales District.
*
...
*    PERFORM list_output.
*  ELSE.
****** Start Code: Added Code by CS on 09.10.2015 for Plant & Sales District Auth. ******
*    IF lv_werks_auth_flg EQ 'X'." OR lv_cityc_auth_flg EQ 'X'.
...
*    ENDIF.
**    MESSAGE s843.
****** End Code: Added Code by CS on 09.10.2015 for Plant & Sales District Auth. ******
*
...
*
******  Start Code: Commented by CS 20.10.2015. *****
...
**      ENDIF.
******  End Code: Commented by CS 20.10.2015. *****
*
*    ENDIF.
*
...
*  DATA: t_mat_tmp TYPE TABLE OF ty_mat, " Added by CS on 20.10.2015 For Authorization Plant & Sales District
*        w_mat_tmp TYPE ty_mat.
*
...
*    SELECT mara~matnr werks xchar mtart matkl meins trame umlmc extwg
...
*************************************************************************
*******  Start Code: Added by CS on 5-Oct-2015 For Check Plant Authorization. *****
**  break test1.
**  FREE: t_mat_tmp[].
**  CLEAR: lv_werks_auth_flg.
**  LOOP AT t_mat.
***    CLEAR: lv_werks_auth_flg. " , lv_cityc_auth_flg.
**    AUTHORITY-CHECK OBJECT 'M_MATE_WRK'
**                 ID 'ACTVT' FIELD '03'
**                 ID 'WERKS' FIELD t_mat-werks.
**    IF sy-subrc NE 0.
**      lv_werks_auth_flg = 'X'.
**    ELSE.
**      MOVE-CORRESPONDING t_mat TO w_mat_tmp.
**      APPEND w_mat_tmp TO t_mat_tmp.
**    ENDIF.
**    CLEAR: w_mat_tmp.
**  ENDLOOP.
**  FREE: t_mat[].
**  t_mat[] = t_mat_tmp[].
**  FREE: t_mat_tmp[].
***  IF lv_werks_auth_flg EQ 'X'.
***    MESSAGE 'Authorization Missing.' TYPE 'S' DISPLAY LIKE 'W'.
***  ENDIF.
*******  End Code: Added by CS on 5-Oct-2015 For Check Plant Authorization. *****
*
...
*
**START  CODE BY PUNAM
** ---- Begin code-rewrite on Thursday, February 01, 2018 21:46:14 ---- *
** ---- Purpose: Performance tuning, use global selects followed by reads in loop instead of select singles... ---- *
** ---- Developer: ABAP02 - SaurabhK ---- *
** ---- TR: IRDK930926 ---- *
...
*
*        BEGIN OF wa_csize,
*          matnr  TYPE z6sd038t_depo-matnr,
*          zfkimg TYPE z6sd038t_depo-zfkimg,
*        END OF wa_csize,
*        it_csize LIKE STANDARD TABLE OF wa_csize,
*
*        BEGIN OF wa_div_desc,
*          matnr TYPE mara-matnr,
*          spart TYPE tspat-spart,
*          vtext TYPE tspat-vtext,
*        END OF wa_div_desc,
*        it_div_desc LIKE STANDARD TABLE OF wa_div_desc,
*
*        BEGIN OF wa_ausp_objek,
*          objek TYPE ausp-objek,
*        END OF wa_ausp_objek,
*        it_ausp_objek LIKE STANDARD TABLE OF wa_ausp_objek,
*
*        BEGIN OF wa_inob_objek,
*          objek TYPE inob-objek,
*        END OF wa_inob_objek,
*        it_inob_objek LIKE STANDARD TABLE OF wa_inob_objek,
*
*        BEGIN OF wa_ausp,
...
*        END OF wa_ausp,
*        it_ausp  LIKE STANDARD TABLE OF wa_ausp,
*        it_ausp2 LIKE STANDARD TABLE OF wa_ausp,
*
*        BEGIN OF wa_plant,
...
*        END OF wa_plant,
*        it_plant LIKE STANDARD TABLE OF wa_plant,
*
*        BEGIN OF wa_t005h,
*          bezei TYPE t005h-bezei,
*          cityc TYPE t005h-cityc,
*        END OF wa_t005h,
*        it_t005h LIKE STANDARD TABLE OF wa_t005h,
*
*        BEGIN OF wa_b_mrp,
*          konda TYPE zsd_batch_mrp-konda,
*          spart TYPE zsd_batch_mrp-spart,
*          matnr TYPE zsd_batch_mrp-matnr,
*          charg TYPE zsd_batch_mrp-charg,
*        END OF wa_b_mrp,
*        it_b_mrp LIKE STANDARD TABLE OF wa_b_mrp,
*
*        BEGIN OF wa_konp,
*          kbetr TYPE konp-kbetr,
*          knumh TYPE konp-knumh,
*        END OF wa_konp,
*        it_konp  LIKE STANDARD TABLE OF wa_konp,
*
*        wa_t188t TYPE t188t,
*        it_t188t LIKE STANDARD TABLE OF wa_t188t,
*
*        BEGIN OF wa_inob,
*          cuobj TYPE inob-cuobj,
*          objek TYPE inob-objek,
*        END OF wa_inob,
*        it_inob LIKE STANDARD TABLE OF wa_inob,
*
*        BEGIN OF wa_mvke,
...
*
*    REFRESH it_mch1.
*    SELECT hsdat    " Date of Manufacture
*           vfdat    " Shelf Life Exp. Date
*           cuobj_bm matnr charg
*      FROM mch1
*      INTO TABLE it_mch1
*      FOR ALL ENTRIES IN bestand
*      WHERE matnr = bestand-matnr
...
*
*    REFRESH it_div_desc.
*    SELECT a~matnr a~spart b~vtext
*      FROM mara AS a
*      JOIN tspat AS b
*      ON a~spart = b~spart
*      INTO TABLE it_div_desc
*      FOR ALL ENTRIES IN bestand
*      WHERE matnr =  bestand-matnr
*      AND spras = 'EN'.
*  ENDIF.
...
*    REFRESH it_ausp_objek.
*    LOOP AT it_mch1 INTO wa_mch1.
*      MOVE wa_mch1-cuobj_bm TO wa_ausp_objek-objek.
*      APPEND wa_ausp_objek TO it_ausp_objek.
*      CLEAR: wa_ausp_objek, wa_mch1.
*    ENDLOOP.
*  ENDIF.
*
*  IF bestand[] IS NOT INITIAL.
*    REFRESH it_inob_objek.
*    LOOP AT bestand.
*      CONCATENATE  bestand-matnr bestand-charg INTO wa_inob_objek-objek.
*      APPEND wa_inob_objek TO it_inob_objek.
*      READ TABLE it_div_desc INTO wa_div_desc WITH KEY matnr = bestand-matnr.
*      IF sy-subrc = 0.
*        bestand-spart = wa_div_desc-spart.
*        bestand-vtext = wa_div_desc-vtext.
*      ENDIF.
*      MODIFY bestand.
*      CLEAR: bestand, wa_inob_objek.
*    ENDLOOP.
*  ENDIF.
*
*  IF it_ausp_objek IS NOT INITIAL.
*    REFRESH it_ausp.
*    SELECT atwrt objek atinn
*      FROM ausp
*      INTO TABLE it_ausp
*      FOR ALL ENTRIES IN it_ausp_objek
*      WHERE objek = it_ausp_objek-objek
*      AND atinn = '0000000810'. "'0000000837'."
*  ENDIF.
*
*  REFRESH it_plant.
*  SELECT werks regio cityc vkorg
*    FROM t001w
*    INTO TABLE it_plant
...
*  IF bestand[] IS NOT INITIAL.
*    REFRESH it_b_mrp.
*    SELECT konda spart matnr charg
*      FROM zsd_batch_mrp
*      INTO TABLE it_b_mrp
*      FOR ALL ENTRIES IN bestand
*      WHERE spart = bestand-spart
...
*  SELECT * FROM a605
*    INTO TABLE it_a605
**    FOR ALL ENTRIES IN bestand " Commented since this slows the query, select all with read is much faster
*    WHERE kappl = 'V'
*    AND kschl = 'ZMRP'
**    AND spart = bestand-spart
*    AND kdgrp = '02'
**    AND matnr = bestand-matnr
**    AND konda = bestand-konda
*    AND datab <= sy-datum
*    AND datbi >= sy-datum.
*
*  IF it_a605 IS NOT INITIAL.
*    REFRESH it_konp.
*    SELECT kbetr knumh
*      FROM konp
*      INTO TABLE it_konp
*      FOR ALL ENTRIES IN it_a605
*      WHERE knumh = it_a605-knumh
*      AND kschl = 'ZMRP'.
*  ENDIF.
*
*  REFRESH it_t188t.
...
*    WHERE spras EQ 'E'.
*
*  IF it_inob_objek IS NOT INITIAL.
*    REFRESH it_inob.
*    SELECT cuobj objek
*      FROM inob
*      INTO TABLE it_inob
*      FOR ALL ENTRIES IN it_inob_objek
*      WHERE klart = '023'
*      AND obtab = 'MCH1'
*      AND objek = it_inob_objek-objek.
*  ENDIF.
*
*  IF it_inob IS NOT INITIAL.
*    REFRESH it_ausp_objek.
*    LOOP AT it_inob INTO wa_inob.
*      MOVE wa_inob-objek TO wa_ausp_objek-objek.
*      APPEND wa_ausp_objek TO it_ausp_objek.
*      CLEAR: wa_inob, wa_ausp_objek.
*    ENDLOOP.
*  ENDIF.
*
*  IF it_ausp_objek IS NOT INITIAL.
...
*    FOR ALL ENTRIES IN it_ausp_objek
*    WHERE objek = it_ausp_objek-objek
*    AND atinn IN ( '0000000821', '0000000867', '0000000831' ).
*  ENDIF.
*
*  IF bestand[] IS NOT INITIAL.
*    REFRESH it_mvke.
*    SELECT matnr vkorg mvgr5
*      FROM mvke
*      INTO TABLE it_mvke
*      FOR ALL ENTRIES IN bestand
*      WHERE matnr = bestand-matnr.
...
*  LOOP AT bestand.
*    CLEAR wa_mard.
*    READ TABLE it_mard INTO wa_mard WITH KEY matnr = bestand-matnr
*                                             werks = bestand-werks
*                                             lgort = bestand-lgort.
*    IF sy-subrc = 0.
*      bestand-lgpbe = wa_mard-lgpbe.
*    ENDIF.
*
*    CLEAR wa_mch1.
*    READ TABLE it_mch1 INTO wa_mch1 WITH KEY matnr = bestand-matnr
*                                             charg = bestand-charg.
*    IF sy-subrc = 0.
*      bestand-hsdat    = wa_mch1-hsdat.
*      bestand-vfdat    = wa_mch1-vfdat.
*      bestand-cuobj_bm = wa_mch1-cuobj_bm.
*    ENDIF.
*
*    CLEAR wa_csize.
*    READ TABLE it_csize INTO wa_csize WITH KEY matnr = bestand-matnr.
*    IF sy-subrc = 0.
*      bestand-zfkimg = wa_csize-zfkimg.
*    ENDIF.
*
*    CLEAR wa_ausp.
*    READ TABLE it_ausp INTO wa_ausp WITH KEY objek = bestand-cuobj_bm.
*    IF sy-subrc = 0.
*      bestand-atwrt = wa_ausp-atwrt.
*    ENDIF.
*
*    IF  bestand-vfdat IS NOT INITIAL.
*      bestand-sl_days = bestand-vfdat - sy-datum.
**write : BESTAND-VFDAT to zfinal_date.
...
*      ENDIF.
*      zyear = bestand-vfdat(4).
*      CONCATENATE bestand-ktx zyear INTO bestand-zdate1
*      SEPARATED BY space.
*      CLEAR :zyear.", bestand-ktx.
************end of added code ****************
*    ENDIF.
...
*
*    CLEAR wa_plant.
*    READ TABLE it_plant INTO wa_plant WITH KEY werks = bestand-werks.
*    IF sy-subrc = 0.
*      bestand-cityc = wa_plant-cityc.
*
*      CLEAR wa_t005h.
*      READ TABLE it_t005h INTO wa_t005h WITH KEY cityc = bestand-cityc.
*      IF sy-subrc = 0.
*        bestand-bezei = wa_t005h-bezei.
*      ENDIF.
*    ENDIF.
*
*    CLEAR wa_b_mrp.
*    READ TABLE it_b_mrp INTO wa_b_mrp WITH KEY matnr = bestand-matnr
*                                               spart = bestand-spart
*                                               charg = bestand-charg.
*    IF sy-subrc = 0.
*      bestand-konda = wa_b_mrp-konda.
*    ENDIF.
*
*    CLEAR wa_a605.
*    READ TABLE it_a605 INTO wa_a605 WITH KEY spart = bestand-spart
*                                             regio = wa_plant-regio
*                                             matnr = bestand-matnr
*                                             konda = bestand-konda.
*    IF sy-subrc = 0.
*      CLEAR wa_konp.
*      READ TABLE it_konp INTO wa_konp WITH KEY knumh = wa_a605-knumh.
*      IF sy-subrc = 0.
*        bestand-kbetr = wa_konp-kbetr.
*        bestand-other_kbetr = wa_konp-kbetr.
*      ENDIF.
*    ENDIF.
*
*    CLEAR wa_t188t.
*    READ TABLE it_t188t INTO wa_t188t WITH KEY konda = bestand-konda.
*    IF sy-subrc = 0.
*      bestand-konda_txt = wa_t188t-vtext.
*    ENDIF.
*
*    CLEAR wa_inob.
*    READ TABLE it_inob INTO wa_inob WITH KEY objek = bestand-matnr && bestand-charg.
*    IF sy-subrc = 0.
*      CLEAR wa_ausp.
*      READ TABLE it_ausp2 INTO wa_ausp WITH KEY objek = wa_inob-cuobj.
*      IF sy-subrc = 0.
*        bestand-atwrt_pg = wa_ausp-atwrt.
*      ENDIF.
*    ENDIF.
*
*    CLEAR wa_mvke.
*    READ TABLE it_mvke INTO wa_mvke WITH KEY matnr = bestand-matnr
*                                             vkorg = wa_plant-vkorg.
*    IF sy-subrc = 0.
*      bestand-mvgr5 = wa_mvke-mvgr5.
*
*      CLEAR wa_tvm5t.
*      READ TABLE it_tvm5t INTO wa_tvm5t WITH KEY mvgr5 = bestand-mvgr5.
*      IF sy-subrc = 0.
*        bestand-mvgr5_txt = wa_tvm5t-bezei.
*      ENDIF.
*    ENDIF.
*
*    MODIFY bestand.
...
*  ENDIF.
*
******* Start Code: Added Code by CS on 20.10.2015 for Checking  Sales District Authorization. ******
***  break test1.
***  IF cityc IS NOT INITIAL.
***    IF bestand[] IS NOT INITIAL.
***      DELETE bestand[] WHERE cityc NOT IN cityc.
...
***  ENDIF.
***  IF bestand[] IS NOT INITIAL.
***    CLEAR: lv_cityc_auth_flg.
***    LOOP AT bestand.
***      AUTHORITY-CHECK OBJECT 'ZCITYC'
***                          ID 'ACTVT' FIELD '03'
***                          ID 'CITYC' FIELD bestand-cityc.
***      IF sy-subrc NE 0.
***        lv_cityc_auth_flg = 'X'.
***        DELETE bestand INDEX sy-tabix.
***      ENDIF.
***    ENDLOOP.
***    IF lv_werks_auth_flg EQ 'X' OR lv_cityc_auth_flg EQ 'X'.
...
***    ENDIF.
***  ENDIF.
******* End Code: Added Code by CS on 20.10.2015 for Checking  Sales District Authorization. ******
*
...
*
*  CLEAR fieldcat.
*  fieldcat-fieldname     = 'ZFKIMG'.
*  fieldcat-tabname       = 'BESTAND'.
*  fieldcat-seltext_s     = 'Case Size'.
*  fieldcat-seltext_m    = 'Case Size'.
*  fieldcat-seltext_l     = 'Case Size'.
*  fieldcat-ref_tabname   = 'Z6SD038T_DEPO'.
*  APPEND fieldcat.
*
*
***START PUNAM
...
*  fieldcat-tabname       = 'BESTAND'.
**  fieldcat-ref_tabname   = 'T001W'.
*  fieldcat-seltext_m     = 'Storage Bin'.
*  APPEND fieldcat.
*
*  CLEAR fieldcat.
*  fieldcat-fieldname     = 'ZFKIMG'.
...
*
*  CLEAR fieldcat.
*  fieldcat-fieldname     = 'KONDA_TXT'.
*  fieldcat-tabname       = 'BESTAND'.
**  fieldcat-ref_tabname   = 'T001W'.
*  fieldcat-seltext_m     = 'Price grp.'.
*  APPEND fieldcat.
*
*  CLEAR fieldcat.
...
*  fieldcat-tabname       = 'BESTAND'.
**  fieldcat-ref_tabname   = 'T001W'.
*  fieldcat-seltext_m     = 'Condition amt.'.
*  APPEND fieldcat.
*
*  CLEAR fieldcat.
*  fieldcat-fieldname     = 'ATWRT_PG'.
*  fieldcat-tabname       = 'BESTAND'.
**  fieldcat-ref_tabname   = 'T001W'.
*  fieldcat-seltext_m     = 'OTHER PG MSC2N/COR2'.
*  APPEND fieldcat.
*
*  CLEAR fieldcat.
*  fieldcat-fieldname     = 'ATWRT_TXT'.
*  fieldcat-tabname       = 'BESTAND'.
**  fieldcat-ref_tabname   = 'T001W'.
*  fieldcat-seltext_m     = 'OTHER PG'.
*  APPEND fieldcat.
*
*
*  CLEAR fieldcat.
*  fieldcat-fieldname     = 'OTHER_KBETR'.
*  fieldcat-tabname       = 'BESTAND'.
**  fieldcat-ref_tabname   = 'T001W'.
*  fieldcat-seltext_m     = 'MSC2N/COR2 Cond.amt.'.
*  APPEND fieldcat.
*
*
*  CLEAR fieldcat.
...
*  fieldcat-tabname       = 'BESTAND'.
**  fieldcat-ref_tabname   = 'T001W'.
*  fieldcat-seltext_m     = 'Material Grp.5'.
*  APPEND fieldcat.
*
*  CLEAR fieldcat.
*  fieldcat-fieldname     = 'MVGR5_TXT'.
*  fieldcat-tabname       = 'BESTAND'.
**  fieldcat-ref_tabname   = 'T001W'.
*  fieldcat-seltext_m     = 'Material Grp.5'.
*  APPEND fieldcat.
...
*
*
** KONDA type ZSD_BATCH_MRP-KONDA, " Price group (customer)
**       kbetr type konp-kbetr,   " Rate (condition amount or percentage) where no scale exists
**       KONDA_TXT type T188T-VTEXT.   " PRICING GRP TXT
** END PUNAM
*
*
...
**&      Form  CHECK_AUTH_OBJ
**&---------------------------------------------------------------------*
** Added by Chirag Shah 19.10.2015 Check Authority of Plant & Sales District..
**----------------------------------------------------------------------*
**  -->  p1        text
**  <--  p2        text
**----------------------------------------------------------------------*
...
*        .
*
**  DATA: w_s_werks LIKE LINE OF werks, " Work Area for Plant Select-Option
**        w_s_cityc LIKE LINE OF cityc. " Work Area for Sales District Select-Option
...
*
*  break test1.
****** Start Code: Added by CS on 19.10.2015 for Plant Authorization. *****
...
*    CLEAR: werks.
*  ENDIF.
****** End Code: Added by CS on 19.10.2015 for Plant Authorization. *****
*
******* Start Code: Added by CS on 19.10.2015 for Sales District/City - 4 Char Authorization. *****
...
**  ENDIF.
*  IF lv_auth_flg EQ 'X'.
*    MESSAGE 'Authorization Missing for plant.' TYPE 'S' DISPLAY LIKE 'W'.
*  ENDIF.
******* End Code: Added by CS on 19.10.2015 for Sales District/City - 4 Char Authorization. *****
