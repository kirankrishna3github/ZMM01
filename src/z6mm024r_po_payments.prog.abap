...
*   REQUEST #: IRDK914352 (Already Created by someone)
* --------------------------------------------------------------------------------------------*
* REVISION HISTORY----------------------------------------------------------------------------*
*
*   CHANGED BY: Chirag Shah
...
START-OF-SELECTION.
  ...
  PERFORM check_auth_obj.  " Added by CS on 12.10.2015 for Authorization.

  PERFORM get_data.

*  PERFORM display-output TABLES itab.
***** Start Code: Added by CS on 12.10.2015 for Authorization. *****
*  PERFORM display_data.  " Commented by CS on 12.10.2015
  IF iekbe[] IS NOT INITIAL.
    IF lv_werks_auth_flg = 'X' OR lv_bukrs_auth_flg = 'X'.
      MESSAGE 'Missing Authorization for Company Code/Plant.' TYPE 'S' DISPLAY LIKE 'W'.
...
*    'IEKBE' 'INVDT' 'Inv.Date'   'BUDAT'   'EKBE' 18 fieldtab,    "document date and Invoice date is interchanged
    'IEKBE' 'INDDT' 'Inv.Date'   'BUDAT'   'EKBE' 18 fieldtab,
...
*    'IEKBE' 'INDDT' 'Document Date'   'BUDAT'   'EKBE' 21 fieldtab,  "document date and Invoice date is interchanged
    'IEKBE' 'INVDT' 'Document Date'   'BUDAT'   'EKBE' 21 fieldtab,

    'IEKBE' 'PAYDT' 'Payment.Date'   'BUDAT'   'EKBE' 22 fieldtab,
    'IEKBE' 'INV_DUE_DT' 'Inv.Due.Date'   ''   '' 22 fieldtab,           "Added by Anees 2/04/2010
    'IEKBE' 'PAYMT' 'Payment.Amount'   'DMBTR'   'EKBE' 24 fieldtab,
    'IEKBE' 'WAERS' 'Curr'   'WAERS'   'MSEG' 25 fieldtab,
    'IEKBE' 'MEINS' 'UOM'   'MEINS'   'MSEG' 26 fieldtab,
    'IEKBE' 'XBLNR' 'Delivery.Note'   'XBLNR'   'MKPF' 27 fieldtab.      "Added by Anees 3/23/2010
...
FORM initialize_variant.
  ...
  CALL FUNCTION 'REUSE_ALV_VARIANT_DEFAULT_GET'
    ...
    EXCEPTIONS
      not_found  = 2.
  IF sy-subrc = 0.
*    p_vari = gx_variant-variant. " Added by CS on 12.10.2015
    p_var = gx_variant-variant.   " Added by CS on 12.10.2015
  ENDIF.
...
*            WITH SY-MSGV1 SY-MSGV2 SY-MSGV3 SY-MSGV4.
  ELSE.
    IF g_exit = space.
*      p_vari = gx_variant-variant. " Commented the line by CS on 12.10.2015
      p_var = gx_variant-variant. " Added by CS on 12.10.2015
    ENDIF.
  ENDIF.
...
***** Start Code: Added by CS on 12.10.2015 for Plant Authorization. *****
  ...
  IF s_werks[] IS INITIAL.
    ...
    CLEAR: s_werks.
  ENDIF.
***** End Code: Added by CS on 12.10.2015 for Plant Authorization. *****

***** Start Code: Added by CS on 12.10.2015 for Company Code Authorization. *****
  ...
  IF s_bukrs[] IS INITIAL.
    ...
    CLEAR: s_bukrs.
  ENDIF.
***** End Code: Added by CS on 12.10.2015 for Company Code Authorization. *****
