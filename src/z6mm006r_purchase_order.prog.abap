...
*--------------------------------------------------------------------*
*}   INSERT
REPORT  Z6MM006R_PURCHASE_ORDER.
TABLES: NAST ,
        EKPO ,
        EKKO.
...
DATA : I_KONV TYPE PRCD_ELEMENTS OCCURS 0 WITH HEADER LINE.
*}   REPLACE
...
*----------------------------------------------------------------------*
FORM PRINT_SMART_FORM .
  DATA: LV_BEDAT   TYPE BEDAT VALUE '20170701'.
  DATA: LF_FM_NAME TYPE RS38L_FNAM.
  READ TABLE I_EKPO INDEX 1.

  READ TABLE I_EKKO INTO WA_EKKO INDEX 1.
  IF SY-SUBRC = 0.
    IF WA_EKKO-BEDAT LT LV_BEDAT AND NAST-KSCHL EQ 'NEU'.
      ...
      ELSEIF WA_EKKO-BSART = 'ZIMP'.
        ...
        CALL FUNCTION 'SSF_FUNCTION_MODULE_NAME'
               EXPORTING  FORMNAME           = 'Z6MM006S_IMPORT_PO_PRINT'
                IMPORTING  FM_NAME            = LF_FM_NAME
                EXCEPTIONS NO_FORM            = 1
                           NO_FUNCTION_MODULE = 2
                           OTHERS             = 3.
      ...
      ENDIF.
    ELSE.
      MESSAGE |Purchase Order Created After July 17 - Invalid Output Type| TYPE 'E'.
    ENDIF.
  ENDIF.
