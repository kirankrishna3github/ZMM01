...
*  from EKBE ( PO History )
  IF r_gr = 'X' ."AND IT_EKPO[] IS NOT INITIAL.
    SELECT ebeln ebelp vgabe gjahr belnr budat dmbtr buzei menge bamng shkzg bewtp
      FROM ekbe
      INTO TABLE it_ekbe
*      FOR ALL ENTRIES IN IT_EKPO
...
*            EBELP = IT_EKPO-EBELP AND
    ...
    SELECT a~ebeln a~ebelp a~vgabe a~gjahr a~belnr a~budat a~dmbtr a~buzei a~menge a~bamng a~shkzg a~bewtp
      FROM ekbe AS a
      JOIN ekko AS b
      ON a~ebeln = b~ebeln
...
*            EBELP = IT_EKPO-EBELP AND
    ...
    IF it_ekbe IS NOT INITIAL.
      SELECT mblnr mjahr zeile matnr werks menge meins erfmg erfme  " Added by CS on 04.05.2016 for SKU UOM
        FROM mseg
        INTO TABLE it_mseg
        FOR ALL ENTRIES IN it_ekbe
        WHERE mblnr EQ it_ekbe-belnr
          AND mjahr EQ it_ekbe-gjahr
          AND zeile EQ it_ekbe-buzei.
...
FORM fill_fieldcat .
  ...
  IF r_gr = 'X'.
    PERFORM get_fieldcat USING :
      ...
      'BAMNG' 'IT_OUTPUT' 'MENGE' text-103,"10. " Qty in SKU Added by CS on 04.05.2016
      'BAMNG_MEINS' 'IT_OUTPUT' 'MEINS' text-104."10. " SKU UOM Added by CS on 04.05.2016
...
*      'MPRICE'    'KONV' 'KWERT' text-016,
    ...
    'LBT'           'IT_OUTPUT' 'KWERT' text-096, "10,
    'ZETX'          'IT_OUTPUT' 'KWERT' text-097, "10,
    'ZTC3'          'IT_OUTPUT' 'KWERT' text-098, "10,
    'ZSER'          'IT_OUTPUT' 'KWERT' text-099, "10,
    'ZOC2'          'IT_OUTPUT' 'KWERT' text-100, "10,
    'ZFBT'          'IT_OUTPUT' 'KWERT' text-101, "10,
    'ZPK3'          'IT_OUTPUT' 'KWERT' text-102 "10,
.
*  Import PO
  CLEAR wa_ekko.
...
***** Start Code: Added by CS on 07.01.2016 *****
      ...
      WHEN 'ZBPC'.
        ...
        ENDIF.
      WHEN 'ZLBT'.
        wa_output-lbt = wa_konv-kwert.  " Added by CS on 29.04.2016
      WHEN 'ZTC3'.
        wa_output-ztc3 = wa_konv-kwert. " Added by CS on 03.05.2016
      WHEN 'ZETX'.
        wa_output-zetx = wa_konv-kwert. " Added by CS on 03.05.2016
      WHEN 'ZSER'.
        wa_output-zser = wa_konv-kwert. " Added by CS on 04.05.2016
      WHEN 'ZOC2'.
        wa_output-zoc2 = wa_konv-kwert. " Added by CS on 04.05.2016
      WHEN 'ZFBT'.
        wa_output-zfbt = wa_konv-kwert. " Added by CS on 04.05.2016
      WHEN 'ZPK3'.
        wa_output-zpk3 = wa_konv-kwert. " Added by CS on 04.05.2016
...
*****  Start Code: Added by CS on 04.05.2016 for Qty in SKU & UOM. *****
        wa_output-bamng    = wa_ekbe-bamng. " Qty in SKU  - Added by CS on 04.05.2016

        READ TABLE it_mseg INTO wa_mseg WITH KEY mblnr = wa_ekbe-belnr
                                                 mjahr = wa_ekbe-gjahr
                                                 zeile = wa_ekbe-buzei BINARY SEARCH.
        IF sy-subrc EQ 0.
          wa_output-bamng_meins = wa_mseg-meins.  " SKU UOM - Added by CS on 04.05.2016
        ENDIF.
*****  End Code: Added by CS on 04.05.2016 for Qty in SKU & UOM. *****
...
***** Start Code: Added by CS on 09.05.2016 for Negative Values. SHKZG = 'S' . *****
              IF wa_ekbz-shkzg EQ 'S'.
                CASE wa_ekbz-kschl.
                  WHEN 'JOCM' OR 'ZJO2' OR 'ZLBT' OR 'ZTC3' OR 'ZETX' OR 'ZSER' OR 'ZOC2' OR 'ZFBT' OR 'ZPK3'.
                    wa_ekbz-dmbtr = wa_ekbz-dmbtr * ( -1 ).
                  WHEN OTHERS.
                ENDCASE.
              ENDIF.
***** End Code: Added by CS on 09.05.2016 for Negative Values. SHKZG = 'S' . *****
                ...
                CASE wa_ekbz_sto-kschl.
                  ...
                  WHEN 'ZLBT'.
                    wa_output-lbt             = wa_ekbz_sto-dmbtr.
                  WHEN 'ZTC3'.
                    wa_output-ztc3             = wa_ekbz_sto-dmbtr.
                  WHEN 'ZETX'.
                    wa_output-zetx             = wa_ekbz_sto-dmbtr.
                  WHEN 'ZSER'.
                    wa_output-zser             = wa_ekbz_sto-dmbtr.
                  WHEN 'ZOC2'.
                    wa_output-zoc2             = wa_ekbz_sto-dmbtr.
                  WHEN 'ZFBT'.
                    wa_output-zfbt             = wa_ekbz_sto-dmbtr.
                  WHEN 'ZPK3'.
                    wa_output-zpk3             = wa_ekbz_sto-dmbtr.
                  WHEN OTHERS.
                ENDCASE.
                CLEAR: wa_ekbz_sto.
              ENDLOOP.
...
***** End Code: Added by CS on 09.05.2016 for Negative Values. SHKZG = 'S' . *****
              ...
              CASE wa_ekbz-kschl.
                ...
                WHEN 'ZLBT'.
                  wa_output-lbt             = wa_ekbz-dmbtr.
                WHEN 'ZTC3'.
                  wa_output-ztc3             = wa_ekbz-dmbtr.
                WHEN 'ZETX'.
                  wa_output-zetx             = wa_ekbz-dmbtr.
                WHEN 'ZSER'.
                  wa_output-zser             = wa_ekbz-dmbtr.
                WHEN 'ZOC2'.
                  wa_output-zoc2             = wa_ekbz-dmbtr.
                WHEN 'ZFBT'.
                  wa_output-zfbt             = wa_ekbz-dmbtr.
                WHEN 'ZPK3'.
                  wa_output-zpk3             = wa_ekbz-dmbtr.
                WHEN OTHERS.
              ENDCASE.
              CLEAR: wa_ekbz.
            ENDLOOP.
...
*        CLEAR: WA_OUTPUT-VAT, WA_OUTPUT-CST.  "VAT & CST already included in RM, so clear the amount and display zero in resp columns
        ...
        IF wa_ekko-bsart = 'ZSTO'.
          ...
          SELECT SINGLE dmbtr FROM ekbe INTO wa_output-lcurr
            ...
            AND bewtp = 'U'.
          wa_output-land_cost = wa_output-lcurr + wa_output-fri_amt + wa_output-octori + wa_output-lbt + wa_output-ztc3 + wa_output-zetx
                              + wa_output-zser + wa_output-zoc2 + wa_output-zfbt + wa_output-zpk3.
*        WA_OUTPUT-LAND_COST = WA_OUTPUT-LAND_COSTPU * WA_OUTPUT-GR_MENGE.
        ELSE.
          wa_output-land_cost = wa_output-tot_price - wa_output-tot_exc - wa_output-tot_cd + wa_output-lbt + wa_output-ztc3 + wa_output-zetx
                              + wa_output-zser + wa_output-zoc2 + wa_output-zfbt + wa_output-zpk3.
        ENDIF.
...
FORM check_auth_obj .
  TYPES: BEGIN OF ty_t024e,
        ...
        END OF ty_t134t
          ...
          .
  DATA: t_t024e TYPE TABLE OF ty_t024e, " Pur. Org.
