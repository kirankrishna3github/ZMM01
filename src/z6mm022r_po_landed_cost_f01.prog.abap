...
*  key and currency exchange rate from table EKKO based on user input
  IF r_po = 'X'.
    ...
    SELECT ebeln bedat bsart lifnr waers wkurs knumv zterm rlwrt FROM ekko
      INTO TABLE it_ekko
...
****************************************************************************************************
      SELECT knumv kposn kschl kwert FROM PRCD_ELEMENTS
*}   REPLACE
        INTO TABLE it_konv
        FOR ALL ENTRIES IN it_ekko
...
*  from EKBE ( PO History )
  IF r_gr = 'X' ."AND IT_EKPO[] IS NOT INITIAL.
    ...
    SELECT ebeln ebelp gjahr belnr budat dmbtr buzei menge shkzg FROM ekbe
  INTO TABLE it_ekbe
*      FOR ALL ENTRIES IN IT_EKPO
  WHERE ebeln IN so_ebeln "= IT_EKPO-EBELN AND
*            EBELP = IT_EKPO-EBELP AND
        AND budat IN so_budat AND
        bewtp IN ('E').

    SELECT a~ebeln a~ebelp a~gjahr a~belnr a~budat a~dmbtr a~buzei a~menge a~shkzg
      FROM ekbe AS a
      JOIN ekko AS b
      ON a~ebeln = b~ebeln
...
*            EBELP = IT_EKPO-EBELP AND
    ...
    IF it_ekbe_sto IS NOT INITIAL.
      SELECT * FROM ekbz
        ...
        WHERE belnr = it_ekbe_sto-belnr.
      IF sy-subrc = 0.
        SORT it_ekbz_sto BY belnr.
      ENDIF.

    ENDIF.
    IF it_ekbe IS NOT INITIAL.

      SELECT * FROM ekbz INTO TABLE it_ekbz
        FOR ALL ENTRIES IN it_ekbe
        WHERE belnr = it_ekbe-belnr.

      IF sy-subrc = 0.
        SORT it_ekbz BY belnr.
      ENDIF.
      SELECT * FROM j_1igrxref INTO TABLE it_j_1igrxref
      FOR ALL ENTRIES IN it_ekbe
