      ...
      IF SY-SUBRC = 0.
        IF I_EKPO-WERKS = '1101'.
          DELETE I_KONV WHERE NOT ( KSCHL EQ 'PB00'
              ...
              OR   KSCHL EQ 'ZPK3'
              OR   KSCHL EQ 'FRC1'
              OR   KSCHL EQ 'IIN2'     " added by NK on 22.09.2017
              ).
        ENDIF.
      ENDIF.
    ELSE.
...
FORM PRINT_SMART_FORM .
  ...
  DATA: LF_FM_NAME TYPE RS38L_FNAM,
        ...
        LV_BEDAT   TYPE BEDAT VALUE '20170701'.
  READ TABLE I_EKPO INDEX 1.
  BREAK: ABAP01,10106.
  ...
  READ TABLE I_EKKO INTO WA_EKKO INDEX 1.
    IF SY-SUBRC = 0.
**      IF WA_EKKO-BUKRS = '3000'.
**        IF WA_EKKO-BSART = 'ZSTO'.
**          LV_FNAME = 'Z6MM006S_STOCK_TRANS_ORDER'.
**
**        ELSEIF WA_EKKO-BSART = 'ZIMP'.
**          LV_FNAME = 'ZMM_IMPORT_PO_GST_SF'.
**
**        ELSE.
**          if wa_ekko-bukrs = '2800'.
**            LV_FNAME = 'ZMM_PURCHASE_ORDER_INDOREAGENS'.
**
**          ELSE.
**            LV_FNAME = 'ZMM_PURCHASE_ORDER_FORM'.
**          ENDIF.
**        ENDIF.
**      ELSE.
**        IF WA_EKKO-BEDAT LT LV_BEDAT AND NAST-KSCHL EQ 'Z001'.
**          " Not Print
**          MESSAGE |Purchase Order Created Before July 17 - Invalid OUTPUT Type.| TYPE 'E'.
**
**        ELSEIF WA_EKKO-BEDAT GE LV_BEDAT AND NAST-KSCHL EQ 'Z001'.
        IF WA_EKKO-BEDAT GE LV_BEDAT AND NAST-KSCHL EQ 'Z001'.
          " Print
          IF WA_EKKO-BSART = 'ZSTO'.
            LV_FNAME = 'Z6MM006S_STOCK_TRANS_ORDER'.
...
**
**       CALL FUNCTION 'SSF_FUNCTION_MODULE_NAME'
**                 EXPORTING  FORMNAME            = 'ZMM_PURCHASE_ORDER_INDOREAGENS'
**                  IMPORTING  FM_NAME            = LF_FM_NAME
**                  EXCEPTIONS NO_FORM            = 1
**                             NO_FUNCTION_MODULE = 2
**                             OTHERS             = 3.
